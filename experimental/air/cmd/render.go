...
// progressBarWidth is the fixed glyph count of the progress bar. Everything else
// in the view is data-driven; only the bar has a layout constant.
const progressBarWidth = 34

// Box titles, rendered into the top border of each box.
const (
	configBoxTitle   = "Configuration"
	progressBoxTitle = "Training Progress"
...
func renderRunText(ctx context.Context, out io.Writer, w *databricks.WorkspaceClient, run *jobs.Run, data *getData, ids *mlflowIdentifiers) {
	...
	var progress mlflowRunDetails
	if ids != nil {
		progress = fetchMLflowRun(ctx, w, ids.RunID)
		view.mlflowLabel = mlflowRunLabel(progress.Name, ids.RunID)
	...
	// A failed run made no meaningful progress, so omit the progress box.
	if !isFailedStatus(view.status) {
		bar := renderProgressBar(p, view.status, progress.Done, progress.Total, progress.HasSteps, view.duration)
		sections = append(sections, renderBox(p, progressBoxTitle, bar))
	}
...
// renderProgressBar draws the bar, percent, and step/duration line. A successful
// run is always full; an in-progress run with step data fills floor(34*done/total).
// Without step data the bar reflects status and the steps segment is dropped.
func renderProgressBar(p palette, status string, done, total int, hasSteps bool, duration string) string {
	fill := statusStyle(p, status)

	var fraction float64
	switch {
	case isSuccessStatus(status):
		fraction = 1
	case hasSteps && total > 0:
		fraction = min(float64(done)/float64(total), 1)
	}

	filled := min(int(math.Floor(progressBarWidth*fraction)), progressBarWidth)
	bar := fill.Render(strings.Repeat("█", filled)) + p.n7.Render(strings.Repeat("░", progressBarWidth-filled))

	segments := []string{bar}
	if hasSteps || isSuccessStatus(status) {
		segments = append(segments, p.n12.Render(strconv.Itoa(int(math.Round(fraction*100)))+"%"))
	}
	switch {
	case hasSteps:
		segments = append(segments, p.n7.Render(fmt.Sprintf("%d/%d steps · %s", done, total, duration)))
	case duration != "" && duration != na:
		segments = append(segments, p.n7.Render(duration))
	}
